...
export const SPOTIFY_RECOMMENDATIONS_URI = 'https://api.spotify.com/v1/recommendations';
...
export const getSongRecommendations = async (artistIds, genres, trackIds, emotionalScore) => {
    ...
    return axios.get(
        SPOTIFY_RECOMMENDATIONS_URI + params,
        { headers }
        )
...
const _buildParams = (artistIds, genres, trackIds, emotionalScore = null) => {
    const queryParamString = `?seed_artists=${artistIds.join()}&seed_genres=${genres.join()}&seed_tracks=${trackIds.join()}`
